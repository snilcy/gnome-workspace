...
function buildPrefsWidget() {
  ...
  //   settings.set_int('position', w.get_value_as_int());
  // });
  ...
  //   label:
  //     '<span foreground="red"><i><b>Close settings to apply modifications below this label (working on a fix)</b></i></span>',
  ...
  //   settings.set_string('indicators-color', widget.get_rgba().to_string());
  // });
  // gridWidget.attach(label, 0, 12, 1, 1);
  // gridWidget.attach(widget, 1, 12, 1, 1);

  // // apps-on-all-workspaces-indicator
  // const oldValue = settings.get_string('apps-on-all-workspaces-indicator');
  // label = new Gtk.Label({
  ...
  // widget.connect('icon-press', (e) => e.set_text(oldValue));
  // widget.is_entry = true;
  // widget.set_text(oldValue);
  // widget.connect('changed', (e) => {
  //   settings.set_string(
  //     'apps-on-all-workspaces-indicator',
  //     e.get_text().length > 0 ? e.get_text() : oldValue,
  //   );
  // });
